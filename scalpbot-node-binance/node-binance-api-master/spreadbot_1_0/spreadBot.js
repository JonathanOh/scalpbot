...
const getRunTime = function(callback) {
  ...
  globalData.clock._d = (d < 10 ? '' : '') + Number(d);

  h = Math.floor((seconds-d*86400)/3600);
  globalData.clock._h = (h < 10 ? '' : '') + Number(h);

  m = Math.floor((seconds-(d*86400+h*3600))/60);
  globalData.clock._m = (m < 10 ? '' : '') + Number(m);

  s = Math.floor(seconds-(d*86400+h*3600+m*60));
  globalData.clock._s = (s < 10 ? '' : '') + Number(s);

	globalData.clock.profitPerDay = (86400 / Number(seconds)) * Number(globalData.accountStats.netBalance);
}
	...
	if(!stateProcessing) {
		...
		switch(state)
		{
			...
			case 4: // MARKET ENTRY VALIDATION [SPREAD]
				...
				console.log("  -------------------------------------");
				console.log("    > Run time                 : " + FgBrightWhite + globalData.clock._d + "d " + globalData.clock._h + "h " + globalData.clock._m + "m " + globalData.clock._s + "s" + Reset);
				...
				if (globalData.accountStats.netBalance >= 0) {
					...
					console.log("    > Profit / Day: " + FgBrightGreen + Number(globalData.clock.profitPerDay).toFixed(2) + Reset);
				} else {
			...
			case 10: // [STAGE 1] MONITOR OPEN ORDER
				...
				updateOrderStatus(function() {
					...
					if (globalData.ordering.order.status == 'FILLED') {
						...
						state = 19; // order canceled, end cycle
					} else {
						console.log("   --- ");
			...
			case 16: // [STAGE 2] CANCEL ORDER [LOSS-CUT]
				...
				cancelOrder(function(response) {
					...
					if (response) {
								...
								getAccountBalances(function() {
									if (config.sellFirst) {
										if (globalData.accountBalances.ethBalance < (config.stageTwoMinimumCoinAmount * globalData.ordering.targetAskPrice)) {
											...
											state = 19; // cycle complete, go to profit/loss screen
										} else {
						...
						state = 19; // report error and halt
					}
