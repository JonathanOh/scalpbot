...
module.exports = function() {
	...
	return {
		...
		pingServer: function(callback) {
			request(base+"v1/ping", function(error, response, body) {
				if ( !response || !body ) throw "pingServer error: "+error;
				if ( callback && body == "{}") {
					callback(1);
				} else {
					callback(0);
		...
		serverTime: function(callback) {
			request(base+"v1/time", function(error, response, body) {
				if ( !response || !body ) throw "serverTime error: "+error;
				if ( callback ) callback(JSON.parse(body));
			});
		},
		...
		depthv2: function(symbol, callback, limit) {
			publicRequest(base+"v1/depth", {symbol:symbol, limit:limit}, function(data) {
				return callback.call(this, depthData(data), symbol);
			});
		},		
